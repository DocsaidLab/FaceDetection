    ...
    if len(boxes):
        ...
        for box, score, kpts in zip(boxes, scores, kpts_list):
            ...
            if score is not None:
                ...
                text_size = np.clip(round(0.05 * box.height), 4, 24) * draw_scale
                if show_score:
                    plotted = cb.draw_text(
                        plotted,
                        f"{score: .3f}",
                        location=box.left_top,
                        color=tuple(color[::-1]),
                        text_size=text_size,
                        stroke_width=0,
                        stroke_fill=(255, 255, 255),
                    )
            thickness = np.clip(round(0.005 * box.height) + 1, 1, 2) * draw_scale
            plotted = cb.draw_box(plotted, box, color=tuple(color), thickness=thickness)

            if kpts is not None:
                scale = np.clip(box.height / img_h, 0.1, 1.5) * 2 * draw_scale
